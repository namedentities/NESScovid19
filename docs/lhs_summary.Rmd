...
pd <- lhs_long_clean %>% dplyr::select(wikidata_id,dataset) %>% distinct()
library(party)
temp_list1 <- list()
for(i in 1:nrow(pd)){
      p <- pd$wikidata_id[i]
      ...
      try({
        ...
        tryCatch({  
          tree_confirmed <- party::mob(confirmed_log ~ 1 + date_asnumeric    | date_asnumeric , control = mob_control(minsplit = 3, alpha=.2), data = temp,  model = linearModel)  # + I(date_asnumeric^2)     + I(t^3)  # + I(t^2) + I(t^3) + I(t^4) + I(t^2) 
          ...
          temp <- temp %>% 
                  ...
                  mutate(confirmed_log_y_hat_slope=difference(confirmed_log_y_hat)) %>% 
                  fill(confirmed_log_y_hat_slope, .direction="up") %>% 
                  ungroup() %>% mutate(confirmed_log_y_hat_percent_change = round((exp(confirmed_log_y_hat_slope)-1)*100,2))  %>%
                  filter(confirmed_log_y_hat>0) #reject any where the prediction is less than 1 full case
        }, error = function(e) {})
        
        tryCatch({  
          tree_deaths <- party::mob(deaths_log ~ 1 + date_asnumeric    | date_asnumeric , control = mob_control(minsplit = 3, alpha=.2), data = temp,  model = linearModel)  # + I(date_asnumeric^2)     + I(t^3)  # + I(t^2) + I(t^3) + I(t^4) + I(t^2) 
          ...
          temp <- temp %>% group_by(deaths_log_group_number) %>% arrange(date_asnumeric) %>% mutate(deaths_log_y_hat_slope=difference(deaths_log_y_hat)) %>% fill(deaths_log_y_hat_slope, .direction="up") %>% ungroup() %>%
                              ...
                              filter(deaths_log_y_hat>0) #reject any where the prediction is less than 1 full case
        }, error = function(e) {})
        
        tryCatch({  
          tree_tested_people <- party::mob(tested_people_log ~ 1 + date_asnumeric    | date_asnumeric , control = mob_control(minsplit = 3, alpha=.2), data = temp,  model = linearModel)  # + I(date_asnumeric^2)     + I(t^3)  # + I(t^2) + I(t^3) + I(t^4) + I(t^2) 
          ...
          temp <- temp %>% group_by(tested_people_log_group_number) %>% 
                     arrange(date_asnumeric) %>%
                     mutate(tested_people_log_y_hat_slope=difference(tested_people_log_y_hat)) %>% 
                     ...
                     filter(tested_people_log_y_hat>0) #reject any where the prediction is less than 1 full case
        }, error = function(e) {})
        
        temp_list1[[paste(p,d) ]] <- temp
...
dim(lhs_long_clean_imputed1) #bigger because we're interpolating now
...
for(p in places){
  print(p)
  temp <- lhs_long_clean_imputed1 %>% filter(wikidata_id==p) %>% arrange(date_asnumeric)
          
  tryCatch({  
    rt1 <- rpart::rpart(formula = confirmed_log_y_hat_percent_change ~ date_asnumeric, data=temp, control = rpart.control(minsplit = minsplit, minbucket=minbucket))
    temp$confirmed_log_y_hat_percent_change_y_hat <-  predict(rt1, newdata=temp)
  }, error = function(e) {})
  
  tryCatch({  
    rt2 <- rpart::rpart(formula = deaths_log_y_hat_percent_change ~ date_asnumeric, data=temp, control = rpart.control(minsplit = minsplit, minbucket=minbucket))
    temp$deaths_log_y_hat_percent_change_y_hat <-  predict(rt2, newdata=temp)
  }, error = function(e) {})

  tryCatch({  
    rt3 <- rpart::rpart(formula = tested_people_log ~ date_asnumeric, data=temp)
...
test <- lhs_long_clean_imputed %>% mutate(group=paste(dataset,wikidata_id,confirmed_log_y_hat_percent_change)) %>% filter(wikidata_id=="Q148") %>% 
