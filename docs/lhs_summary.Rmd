...
df_slopes <- temp_df %>% group_by(wikidata_id) %>% 
             arrange(date_asdate) %>% 
             filter(!duplicated(date_asdate)) %>% 
             mutate(confirmed_log_y_hat_percent_change_fd=difference(confirmed_log_y_hat_percent_change)) %>%
             mutate(t = row_number()) %>%
             mutate(t_alligned = ifelse(confirmed_log_y_hat_percent_change_fd==max(confirmed_log_y_hat_percent_change_fd,na.rm=T), t, NA) ) %>%
...
ggplot() + 
   geom_line( aes(x=t_alligned , y=confirmed_log_y_hat_percent_change, group=group), alpha=.05, color="blue") +
   theme_bw() +
   ggtitle("Interpolated log count by Alligned T") + ylim(0,.5)
...
ggplot() + 
   geom_line( aes(x=t_alligned , y=confirmed_log_y_hat_percent_change, group=group, color=wikidata_id), alpha=.5) +
   theme_bw() +
   ggtitle("Interpolated log count by Alligned T") + ylim(0,.5) +
   gghighlight()
...
df_slopes %>% 
  ...
  filter(random>.9) %>%
ggplot() + 
   geom_line( aes(x=t_alligned , y=confirmed_log_y_hat_percent_change, group=wikidata_id), alpha=.2, color="blue") +
   theme_bw() +
   ggtitle("Interpolated log count by Alligned T") + ylim(0,.5)
