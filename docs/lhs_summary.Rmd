...
dim(lhs_long_clean) #292,580 #299213 #181,563
...
df_slopes <- lhs_long_clean_imputed %>% 
             group_by(wikidata_id, date_asdate) %>% 
             mutate(
...
mutinformation(X=df_slopes %>% dplyr::select(t,t_alligned,t_alligned_firstconfirmed,t_alligned_100confirmed,confirmed_log_y_hat_percent_change_y_hat) %>% discretize(), method="emp")             
...
p_t_alligned_firstconfirmed <-  df_slopes %>% mutate(group=paste(wikidata_id)) %>% 
                               ...
                               geom_smooth( aes(x=t_alligned_firstconfirmed , y=confirmed_log_y_hat_percent_change_y_hat), span = 0.05) +
                               theme_bw() +
                               ggtitle("Interpolated log count by Alligned T") + ylim(0,100) +
