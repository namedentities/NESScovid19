...
lhs_long_clean <- lhs_long %>%
  ...
  filter( (is.na(confirmed_fd) | confirmed_fd>0)  ) %>% #rejects a lot
...
There's still missalignment between sources. You can see for example CSSE and NYT are identical except shifted by 1
Except that's only true for parts of the series. So as sources start using each other varies and allignment has to be local
...
Interpolate and find slope for confirmed, deaths, and tests
...
saveRDS(lhs_long_clean_imputed2, "/media/skynet2/905884f0-7546-4273-9061-12a790830beb/rwd_github_private/NESScovid19/data_temp/lhs_long_clean_imputed.Rds")
...
dim(lhs_long_clean_imputed)

lhs_long_clean %>% 
        mutate(group=paste(dataset,wikidata_id)) %>% filter(wikidata_id=="Q16861") %>% 
        arrange(date_asdate)  %>%
        mutate(confirmed_log= confirmed %>% log()) %>%
        ggplot() + 
        geom_point( aes(x=date_asdate, y=confirmed_log, color=dataset), alpha=.3)

test <- lhs_long_clean_imputed %>% 
        mutate(group=paste(dataset,wikidata_id,confirmed_log_group_number)) %>% filter(wikidata_id=="Q869") %>% 
        arrange(date_asdate) # %>%
...
p0a <- test %>%
        ggplot() + 
           ...
           #geom_line(data=temp_df2, aes(x=date_asdate, y=y_hat_mean), color="black", linetype = "dashed") +
           theme_bw() +
           ggtitle("Thailand Q869 Confirmed")
...
test <- lhs_long_clean_imputed %>% mutate(group=paste(dataset,wikidata_id,confirmed_log_group_number)) %>%
...
p3b <- test %>%
       ggplot() + 
         ...
         #geom_line(data=temp_df2, aes(x=date_asdate, y=y_hat_mean), color="black", linetype = "dashed") +
         theme_bw() +
   ggtitle("Bexar County Q16861 Confirmed") + ylim(0,100)
...
(p0a + p1a + p2a + p3a + p4a + p5a) / (p0b + p1b + p2b + p3b + p4b + p5b)


#Alachua County (Q488826)

test <- lhs_long_clean_imputed %>% mutate(group=paste(dataset,wikidata_id,confirmed_log_y_hat_percent_change)) %>% filter(wikidata_id=="Q488826") %>% 
        mutate(group=paste(dataset, wikidata_id,confirmed_log_group_number)) %>%
        arrange(date_asdate) #%>%
        #mutate( confirmed_log_y_hat_percent_change = breakpoints(formula=confirmed_log_y_hat_percent_change ~ 1, data=. ) %>% fitted.values() )
...
ggplot() + 
   ...
   #geom_line(data=temp_df2, aes(x=date_asdate, y=y_hat_mean), color="black", linetype = "dashed") +
   theme_bw() +
   ggtitle("Alachua County (Q488826) Confirmed")
...
We can already start to ask some questions like how does rate of growth vary as a function of how many infected people there are/time

```{r}

head(temp_df)
temp_df %>% mutate(group=paste(wikidata_id, dataset)) %>%
ggplot() + 
   geom_line( aes(x=date_asdate , y=confirmed_log_y_hat, group=group), alpha=.01, color="blue") +
   theme_bw() +
   ggtitle("Interpolated log count by date") + ylim(0,12)


temp_df %>% mutate(group=paste(wikidata_id, dataset)) %>%
ggplot() + 
   geom_line( aes(x=date_asdate , y=confirmed_log_y_hat_percent_change, group=group), alpha=.05, color="blue") +
   theme_bw() +
   ggtitle("Interpolated log count by date") + ylim(0,.7)
...
df_slopes %>% 
  ...
  filter(random>.9) %>%
ggplot() + 
   geom_line( aes(x=date_asdate , y=confirmed_log_y_hat_percent_change, group=wikidata_id), alpha=.2, color="blue") +
   theme_bw() +
   ggtitle("Interpolated log count by Alligned T") + ylim(0,.5)
...
df_slopes %>% mutate(group=paste(wikidata_id)) %>% #filter(wikidata_id %in% c('Q30','Q1439','Q668','Q884','Q16861','Q159','Q38')) %>%
ggplot() + 
   geom_line( aes(x=t_alligned , y=confirmed_log_y_hat_percent_change_y_hat, group=group, color=wikidata_id), alpha=.5) +
   theme_bw() +
   ggtitle("Interpolated log count by Alligned T") + ylim(0,100) +
   gghighlight(wikidata_id %in% c('Q30','Q1439','Q668','Q884','Q16861','Q159','Q38'))

#starts right at zero
#Alachua County (Q488826)
df_slopes %>% mutate(group=paste(wikidata_id)) %>% 
ggplot() + 
   geom_line( aes(x=t_alligned , y=confirmed_log_y_hat_percent_change_y_hat, group=group, color=wikidata_id), alpha=.5) +
   theme_bw() +
   ggtitle("Interpolated log count by Alligned T") + ylim(0,100) +
   gghighlight(wikidata_id %in% c('Q488826'))
...
df_slopes %>% filter(confirmed_log_y_hat_percent_change_y_hat>20 & t_alligned<0)   %>% View()


temp_df %>% 
  group_by(wikidata_id) %>%
    arrange(date_asdate) %>%
    mutate(confirmed_log_y_hat_percent_change_fd= difference(confirmed_log_y_hat_percent_change)) %>%
    filter(confirmed_log_y_hat_percent_change_fd==max(confirmed_log_y_hat_percent_change_fd,na.rm=T)  ) %>%
  ungroup() %>% 
  pull(confirmed) %>%
  summary() #the jump happens at a median of 10 and mean of 358
  

#There are some negatives which should be possible.
temp_df %>%
ggplot() + 
   geom_point( aes(x=confirmed_log , y=confirmed_log_y_hat_percent_change_percent_change, color=dataset), alpha=.2) +
   geom_smooth( aes(x=confirmed_log , y=confirmed_log_y_hat_percent_change_percent_change), alpha=.2, span=.05) + #
   #geom_line( aes(x=date_asdate, y=confirmed_log_y_hat, color=dataset, group=group)) + #
   #geom_line(data=temp_df2, aes(x=date_asdate, y=y_hat_mean), color="black", linetype = "dashed") +
   theme_bw() +
   ggtitle("Rate of growth as a function of confirmed")

temp_df %>%
ggplot() + 
   geom_point( aes(x=date_asdate , y=confirmed_log_y_hat_percent_change_percent_change, color=dataset), alpha=.2) +
   geom_smooth( aes(x=date_asdate , y=confirmed_log_y_hat_percent_change_percent_change), alpha=.2, span=.05) + #
   #geom_line( aes(x=date_asdate, y=confirmed_log_y_hat, color=dataset, group=group)) + #
   #geom_line(data=temp_df2, aes(x=date_asdate, y=y_hat_mean), color="black", linetype = "dashed") +
   theme_bw() +
   ggtitle("Rate of growth as a function of date")
...
Fit linear structural breaks model to each place and dataset, get y_hats, breaks, and slopes
This fits piecewise linear approximations to each individual dataset
