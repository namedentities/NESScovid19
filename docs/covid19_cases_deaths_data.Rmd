...
admin0 <- readRDS(paste0(here::here(),"/data_in/admin0.Rds"))
admin1 <- readRDS(paste0(here::here(),"/data_in/admin1.Rds"))
admin2 <- readRDS(paste0(here::here(),"/data_in/admin2.Rds"))
...
dailyConfirmedCount <- temp$dailyConfirmedCount
temp_matrix <- matrix(nrow=sapply(dailyConfirmedCount, length) %>% max(), ncol=length(dailyConfirmedCount)  )
dim(temp_matrix)
for(i in 1:ncol(temp_matrix)){
  q=dailyConfirmedCount[i] %>% unlist()
  temp_matrix[1:length(q),i] <- q
}

rawdata_df <- as.data.frame(temp_matrix) #this got broken by some being longer now for no reason
...
Descriptions: https://github.com/carranco-sga/Mexico-COVID-19/blob/master/CTD/README.md

EDO = represents the total number of cases in the state 
EDO_D: represents the number of confirmed cases that have died in the state.
EDO_S: represents the total number of suspect cases in the state.
...
# load the data
openMX <- read_csv("https://raw.githubusercontent.com/carranco-sga/Mexico-COVID-19/master/Mexico_COVID19_CTD.csv")

# narrow down to the relevant variables
openMX_long <- openMX %>% 
  dplyr::select(-ends_with("_I"), -ends_with("_R"), -ends_with("_L")) %>% 
  dplyr::select(Fecha:ZAC_D) %>%
  pivot_longer(AGU_S:ZAC_D) 

# reformat into date-location
openMX_long <- full_join(
  openMX_long %>% filter(endsWith(name,"_D")) %>% 
    mutate(state = name, deaths = value) %>%
    dplyr::select(-name, -value) %>% 
    mutate(state = str_remove(state,"_D")),
  openMX_long %>% filter(endsWith(name,"_S")) %>% 
    mutate(state = name, suspected = value) %>%
    dplyr::select(-name, -value) %>%
    mutate(state = str_remove(state,"_S"))) %>% 
  full_join(
    openMX_long %>% filter(!endsWith(name,"_D") & !endsWith(name,"_S")) %>%
      mutate(state = name, confirmed = value) %>%
      dplyr::select(-name, -value)) %>%
  full_join(
    admin1 %>%  dplyr::filter(gid0 == "MEX") %>% 
      dplyr::select(admin1_name_clean, ISO31662) %>% unique() %>% 
      dplyr::filter(!duplicated(ISO31662)) %>% 
      mutate(ISO31662 = stringi::stri_sub(ISO31662,-3, -1)) %>% 
      rename(state=ISO31662)
  ) %>% dplyr::select(-state)




openMX_long <- openMX_long %>% mutate(date_asdate = ymd(Fecha)) %>% 
                 mutate(admin0_name_original="Mexico") %>%
                 mutate(admin1_name_original=admin1_name_clean)  %>%
                 mutate(admin2_name_original='')  %>%
                 mutate(admin0_name_clean=admin0_name_original %>% rex_clean()) %>%
                 mutate(admin1_name_clean=admin1_name_original %>% rex_clean()) %>%
                 mutate(admin2_name_clean=admin2_name_original %>% rex_clean()) %>%
                 rex_admin_function()
# Connection FOR MEXICO IS NOT GREAT
# also need to change the one 2 letter one - Campeche?
# https://en.wikipedia.org/wiki/Template:Mexico_State-Abbreviation_Codes

forjoining_openMX <- openMX_long %>% 
                     mutate(dataset="openMX") %>%
...
#https://github.com/snikitin-de/COVID-19
openRUtests <- read_csv("https://raw.githubusercontent.com/snikitin-de/COVID-19/master/covid-19-russia/covid19-tests-and-other.csv")
openRUcases <- read_csv("https://raw.githubusercontent.com/snikitin-de/COVID-19/master/covid-19-russia/covid19-russia-cases.csv")


openRU_long <- full_join(openRUcases, data.frame(Region_ID = c(1L,2L,3L,4L,5L,6L,7L,8L,
                       9L,10L,11L,12L,13L,14L,15L,16L,17L,18L,19L,
                       95L,21L,22L,23L,24L,25L,26L,27L,28L,29L,30L,31L,
                       ...
                       77L,78L,79L,83L,86L,87L,89L,82L,92L),
              mergename = c("Adygea, Republic",
                       "Bashkortostan, Republic","Buryatiya, Republic",
                       "Altai Republic","Dagestan, Republic",
                       "Ingushetia, Republic","Kabardino-Balkar Republic","Kalmykiya, Republic",
                       "Karachay-Cherkess Republic","Karelia, Republic",
                       "Komi Republic","Mari El Republic","Mordovia, Republic",
                       "Sakha (Yakutiya) Republic", 
                       "North Ossetia-Alania","Tatarstan, Republic","Tuva Republic",
                       "Udmurt Republic","khakassiya, republic",
                       "Chechen Republic","Chuvash Republic","Altai Krai","Krasnodar Krai",
                       "Krasnoyarsk Krai","Primorsky Krai","Stavropol Krai",
                       "Khabarovsk Krai","Amur Oblast","Arkhangelsk Oblast",
                       "Astrakhan Oblast","Belgorod Oblast","Bryansk Oblast",
                       "Vladimir Oblast","Volgograd Oblast","Vologda Oblast",
                       "Voronezh Oblast","Ivanovo Oblast","Irkutsk Oblast",
                       "Kaliningrad Oblast","Kaluga Oblast","Kamchatka Krai",
                       "Kemerovo Oblast","Kirov Oblast","Kostroma Oblast",
                       "Kurgan Oblast","Kursk Oblast","Leningrad Oblast",
                       "Lipetsk Oblast","Magadan Oblast","Moscow Oblast",
                       "Murmansk Oblast","Nizhny Novgorod Oblast","Novgorod Oblast",
                       "Novosibirsk Oblast","Omsk Oblast","Orenburg Oblast",
                       "Oryol Oblast","Penza Oblast","Perm Krai","Pskov Oblast",
                       "Rostov Oblast","Ryazan Oblast","Samara Oblast",
                       "Saratov Oblast","Sakhalin Oblast","Sverdlovsk Oblast",
                       "Smolensk Oblast","Tambov Oblast","Tver Oblast","Tomsk Oblast",
                       "Tula Oblast","Tyumen Oblast","Ulyanovsk Oblast",
                       "Chelyabinsk Oblast","Zabaykalsky Krai","Yaroslavl Oblast",
                       "Moscow","Saint Petersburg","Jewish Autonomous Oblast",
                       "Nenets Autonomous Okrug", 
                       "Khanty–Mansi Autonomous Okrug","Chukotka Autonomous Okrug",
                       "Yamalo-Nenets Autonomous Okrug","Republic of Crimea","Sevastopol"))) %>% 
  filter(!is.na(mergename)) %>%
  dplyr::select(Date, Confirmed, Deaths, mergename) 
  
openRU_long <- openRU_long %>% mutate(date_asdate = ymd(Date)) %>%
  mutate(admin0_name_original="Russia") %>%
  mutate(admin1_name_original=`mergename`)  %>%
  mutate(admin2_name_original='')  %>%
  mutate(admin0_name_clean=admin0_name_original %>% rex_clean()) %>%
  mutate(admin1_name_clean=admin1_name_original %>% rex_clean()) %>%
  mutate(admin2_name_clean=admin2_name_original %>% rex_clean()) %>%
...
forjoining_openRU <- openRU_long %>% 
                     mutate(dataset="openRU") %>%
                     dplyr::select(dataset, gid, geonameid, wikidata_id, date_asdate, confirmed=Confirmed, deaths=Deaths)


# There are a couple that still don't seem to merge in right. Zabaykalsky Krai
# openRU_long[openRU_long$mergename == "Zabaykalsky Krai",]

```

```{r}
# WHY DOESN'T zabaykalskykrai merge in admin 2 data?
temp <- temp %>% filter(admin1_name_clean == "zabaykalskykrai")
temp
# doesn't get geonameid1, feature_code1, wikidata_id1, gid1

```

```{r}
admin1_temp <- admin1 %>% filter(admin1_name_clean == "zabaykalskykrai")
admin1_temp
```

```{r}
# the merge is on c("admin1_name_clean", "geonameid0", "wikidata_id0", "gid0", "ISO31661")
temp[1, c("admin1_name_clean", "geonameid0", "wikidata_id0", "gid0", "ISO31661")] == admin1_temp[1, c("admin1_name_clean", "geonameid0", "wikidata_id0", "gid0", "ISO31661")]

```
So the issue is that admin0 gives us the wikidata_id0 Q159 (Russia), but admin1 has zabaykalskykrai with the wikidata_id0 code Q2184 "Russian Soviet Federative Socialist Republic".

```{r}
rbind(
  unique(admin0[,c("wikidata_id0", "gid0")]),
  unique(admin1[,c("wikidata_id0", "gid0")])) %>% 
  unique() %>% count(gid0) %>% arrange(desc(n))
...
pakistan_wikipedia_cases <- df

pakistan_wikipedia_cases_long <- pakistan_wikipedia_cases %>% janitor::clean_names() %>%
                                  mutate(date_asdate = ymd(date)) %>%
                                  mutate(dataset="wikipedia") %>% 
                                  
                                  mutate(admin0_name_clean= "Pakistan" %>% rex_clean()) %>%
                                  mutate(admin1_name_clean= province %>% rex_clean() ) %>%
                                  mutate(admin2_name_clean= '' ) %>%
                                  rex_admin_function() 
                                                  
forjoining_pakistan_wikipedia_cases <- pakistan_wikipedia_cases_long %>% dplyr::select(dataset,gid, geonameid,wikidata_id,date_asdate, confirmed=total_confirmed_cml)  %>%
...
openNGArecovered <- read_csv("https://raw.githubusercontent.com/Kamparia/nigeria-covid19-data/master/data/csv/ncdc-covid19-states-daily-recovered.csv")

openNGAcases <- read_csv("https://raw.githubusercontent.com/Kamparia/nigeria-covid19-data/master/data/csv/ncdc-covid19-states-daily-cases.csv")

openNGAdeaths <- read_csv("https://raw.githubusercontent.com/Kamparia/nigeria-covid19-data/master/data/csv/ncdc-covid19-states-daily-deaths.csv")



openNGA_long <- 
  openNGArecovered %>% pivot_longer(Abia:Zamfara) %>% rename(recovered = value) %>% mutate(name = rex_clean(name)) %>%
  full_join(openNGAcases %>% pivot_longer(Abia:Zamfara) %>% rename(cases = value) %>% mutate(name = rex_clean(name))) %>% 
  full_join(openNGAdeaths %>% pivot_longer(Abia:Zamfara) %>% rename(deaths = value) %>% mutate(name = rex_clean(name))) %>%
  mutate(name = str_replace(name, "fctabuja", "abujafct"))
  

openNGA_long <- openNGA_long %>% mutate(date_asdate = mdy(Date)) %>%
  mutate(admin0_name_original="Nigeria") %>%
  mutate(admin1_name_original=`name`)  %>%
  mutate(admin2_name_original='')  %>%
  mutate(admin0_name_clean=admin0_name_original %>% rex_clean()) %>%
  mutate(admin1_name_clean=admin1_name_original %>% rex_clean()) %>%
...
forjoining_openNGA <- openNGA_long %>% 
                     mutate(dataset="openNGA") %>%
                     dplyr::select(dataset, gid, geonameid, wikidata_id, date_asdate, confirmed=cases, deaths)
...
```{r}
#https://raw.githubusercontent.com/yahoo/covid-19-data/master/data/by-region-2020-01-01.tsv

#https://raw.githubusercontent.com/yahoo/covid-19-data/master/data/by-region-2020-01-01.tsv
month_start=1
month_end=4
dates_to_grab <- apply(expand.grid(str_pad(month_start:month_end, 2, pad = "0") , str_pad(1:31, 2, pad = "0") ), 1, paste, collapse="-") %>% sort()
dates_to_grab_year <- paste0("2020-",dates_to_grab) 


df_list <- list()
for(q in dates_to_grab_year){
  print(q)
  url = paste0("https://raw.githubusercontent.com/yahoo/covid-19-data/master/data/by-region-",q,".tsv")
  try({
    df_list[[url]] <- read_tsv(url)
  })
}

#ok so they give us second level but not country, amazing...
yahoo_df <- bind_rows(df_list)  %>%
              mutate(date_asdate = ymd(referenceDate)) %>% 
              mutate(dataset="yahoo") %>%
              separate(label, into=c('place_a','place_b','place_c'), sep = ",", remove=F) 

yahoo_long_1 <- yahoo_df %>%
                filter(place_b %>% is.na()) %>%
                mutate(admin0_name_clean=place_a %>% rex_clean()) %>% 
                mutate(admin1_name_clean='' %>% rex_clean()) %>% 
                mutate(admin2_name_clean='' %>% rex_clean()) %>%
                rex_admin_function() %>%
                filter(!is.na(geonameid))

yahoo_long_2 <- yahoo_df %>%
                filter(place_b %>% is.na()) %>%
                filter(!label %in% (yahoo_long_1$label %>% unique()) ) %>%
                mutate(admin1_name_clean=place_a %>% rex_clean()) %>% 
                left_join(admin1) %>% #we now have to clumsly do this by hand
                mutate(geonameid=geonameid1, wikidata_id=wikidata_id1, gid=gid1) %>%
                filter(!is.na(geonameid)) 

#don't do this one, it's mostly cities and leads to bad results
#yahoo_long_3 <- yahoo_long %>%
#                filter(place_b %>% is.na()) %>%
#                filter(!label %in% (c(yahoo_long_1$label,yahoo_long_2$label) %>% unique()) ) %>%
##                mutate(admin2_name_clean=place_a %>% rex_clean()) %>% 
#                left_join(admin2)  %>% #we now have to clumsly do this by hand
#                mutate(geonameid=geonameid2, wikidata_id=wikidata_id2, gid=gid2) %>%
#                filter(!is.na(geonameid)) 
  
yahoo_long_4 <- yahoo_df %>%
                filter(place_c %>% is.na() & !place_b %>% is.na() ) %>%
                filter(!label %in% (c(yahoo_long_1$label,yahoo_long_2$label) %>% unique()) ) %>%
                mutate(admin1_name_clean=place_b %>% rex_clean()) %>% 
                mutate(admin2_name_clean=place_a %>% rex_clean()) %>% 
...
forjoining_yahoo <- yahoo_long %>% 
                    dplyr::select(dataset,gid, geonameid, wikidata_id, date_asdate, confirmed=totalConfirmedCases, deaths=totalDeaths, tested_people=totalTestedCases, tested_samples=numTests)
dim(forjoining_yahoo) #117645
...
Sources
https://docs.google.com/spreadsheets/d/1T2cSvWvUvurnOuNFj2AMPGLpuR2yVs3-jdd_urfWU4c/edit#gid=0

```{r}
#devtools::install_github("r-lib/usethis") #had to install the github latest version
library(usethis); #install.packages('usethis')
if (FALSE) {
  use_zip(url="https://coronadatascraper.com/timeseries-tidy.csv.zip", destdir="/media/skynet2/905884f0-7546-4273-9061-12a790830beb/rwd_github_private/NESScovid19/data_temp", cleanup=FALSE)
  
}
library(data.table)
coronadatascraper <- fread("//media/skynet2/905884f0-7546-4273-9061-12a790830beb/rwd_github_private/NESScovid19/data_temp/timeseries-tidy.csv/timeseries-tidy.csv") %>% as_tibble() #readcsv chokes on this for some reason

table(coronadatascraper_long$aggregate)

#there are some interesting merge problems here
#"Antwerp, Flanders, Belgium" links to two qcodes because flanders shows up twice in wikidata
#I don't think it matters because we subest to unique gid, geonames, qcode anyway  but should clean up one day
coronadatascraper_long <- coronadatascraper %>%
                          mutate(dataset="coronadatascraper") %>%
                          pivot_wider( id_cols = NULL, names_from = type, values_from = value, values_fill = NULL, values_fn = NULL) %>%
                          mutate(date_asdate = ymd(date)) %>% 
                          #filter(name=="Antwerp, Flanders, Belgium") %>%
...
dim(coronadatascraper_long) 

forjoining_coronadatascraper <- coronadatascraper_long %>% 
                                 dplyr::select(dataset,gid, geonameid, wikidata_id, date_asdate, confirmed=cases, deaths=deaths, tested_people=tested)
...
#text <- readLines(url)
#unlist(str_extract_all(string=text, pattern="web/([0-9]*)/https"))
#text= '<a href="/web/20200402/https://www.worldometers.info/coronavirus/">2</a>'
#temp_matches <- str_match_all(string=text, pattern="^.*web/([0-9]*)/https.*$")
#internetarchive_history_page <- read_html(url)
#links <- internetarchive_history_page %>% html_nodes("a") %>% html_attr(c('href'))
#titles <- internetarchive_history_page %>% html_nodes("a") %>% html_attr(c('title')) 
#text <- internetarchive_history_page %>% html_nodes("a") %>% html_text() 
#internetarchive_history_page_links <- data.frame(link=links ,  #paste0("https://en.wikipedia.org/",links)
#                                                       title=titles,
#                                                       text=text) %>%
#                                filter(str_detect(link,"oldid") & text!="cur" & text!="prev")
#This json idea stoped working for some reason
#histor_links_list <- list()
#for(q in dates_to_grab_year){
#  try({
#    url <- paste0(wayback_base_url, q)
#    print(url)
#    wayback_history_page <- read_json(url) #the json isn't returning snapshots past #ya for whatever reason the api isn't returning dates past 20200402 right now, not sure why
...
temp <- readLines("http://web.archive.org/web/timemap/link/https://www.worldometers.info/coronavirus/") #we got it, we can grab them all now
history_links_df <- temp %>% as_tibble() %>% separate(col=value, into=c('link','rel','datetime'), sep = ";") %>% mutate(link=str_replace_all(link,"\\<|\\>",""))

#Ah it's a fixed table that just gets included so scraping the whole page doesn't show any variation
#We really don't need every single snapshot, should fix
history_links_df_one_a_day <- history_links_df %>% filter(link %>% str_detect("web/[0-9]+")) %>% mutate(date=substring(link,28,28+7) %>% as.numeric()) %>% filter(!duplicated(date))

tables_list <- list()
for(q in history_links_df_one_a_day  %>% pull(link) %>% unique() %>% as.character() %>% sort() ){
  print(q)
  thepage = readLines(q)
  #temp_html <- read_html(q)
  #temp_tables <- temp_html  %>% html_nodes("table") %>%  html_table(fill = TRUE, header=F) 
...
worldometers_long <- worldometers %>% 
                      ...
                      mutate(confirmed = dplyr::coalesce(cases,feb_10_11_cases,feb_11_cases,feb_4_cases,feb_6_cases,feb_6_feb_7_cases,total_cases) ) %>% #feb_5_cases #feb_8_cases
                      mutate(deaths = dplyr::coalesce(deaths, feb_10_11_deaths,feb_11_deaths,feb_4_deaths,feb_6_deaths,feb_6_feb_7_deaths,todays_deaths,total_deaths) ) %>% #feb_5_deaths #feb_8_deaths
...
temp <- readLines("https://raw.githubusercontent.com/microsoft/Bing-COVID-19-Data/master/data/2020-4-26.csv") #i opened an issue ticket asking them to put latest but for now we'll have to change this every day
...
lhs_long <- bind_rows(
  ...
  ## South Asia
    ...
    #Pakistan,
    forjoining_pakistan_wikipedia_cases,
...
dim(lhs_long) #650,353 #416,897 #409957 #398999 #310169 #304,376
