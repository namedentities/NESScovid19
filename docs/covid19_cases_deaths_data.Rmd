---
title: "How to be Careful with COVID-19 Counts: What We can and Cannot Learn from Observational Data"
output:
  html_notebook:
    toc: yes
date: 
...
if(fromscratch){
  ...
  dim(countries_df_unique) #16,150 
  countries_df_unique <- bind_rows(
    gadm36_df %>% dplyr::select(gid=GID_0, name=NAME_0 ) %>% distinct(),
    countries_df_unique
  ) %>% distinct()
  ...
  level1 <- gadm36_df %>% dplyr::select(gid=GID_1, a=GID_0, b=GID_1) %>% filter(!is.na(a) & !is.na(b)) %>% 
            left_join(temp5 %>% dplyr::select(a=gid, name_a=name) %>% filter(!is.na(a) & !is.na(name_a) & name_a!='')  ) %>% 
            left_join(temp5 %>% dplyr::select(b=gid, name_b=name) %>% filter(!is.na(b) & !is.na(name_b) & name_b!='')  ) %>%
            mutate(name=paste0(name_a,"_",name_b)) %>% dplyr::select(gid, name) %>% distinct() %>% filter(!is.na(name) & name!='') #%>% filter(!duplicated(name))
  dim(level1) #1,093,517  #1,081,957
  #level1_distinct <- level1 %>% distinct() #no dupes
  
  level2 <- gadm36_df %>% dplyr::select(gid=GID_2, a=GID_0,b=GID_1,c=GID_2) %>% filter(!is.na(a) & !is.na(b) & !is.na(c)) %>% 
            left_join(temp5 %>% dplyr::select(a=gid, name_a=name) %>% filter(!is.na(a) & !is.na(name_a) & name_a!='') ) %>% 
            left_join(temp5 %>% dplyr::select(b=gid, name_b=name) %>% filter(!is.na(b) & !is.na(name_b) & name_b!='') ) %>% 
            left_join(temp5 %>% dplyr::select(c=gid, name_c=name) %>% filter(!is.na(c) & !is.na(name_c) & name_c!='') ) %>%
            mutate(name=paste0(name_a,"_",name_b,"_",name_c)) %>% dplyr::select(gid, name) %>% distinct() %>% filter(!is.na(name) & name!='') %>% filter(!duplicated(name))
  dim(level2) #49,404,935 #49,926,378
  
  level0to3 <- rbindlist(list(level0,level1,level2))
  dim(level0to3) #50,500,567
  level0to3 <- level0to3[!duplicated(name),]
  dim(level0to3) #52,281,025  #50,500,567 
...
geonames_p <- geonames %>% filter(feature_class=="P")
...
bing_long <- bind_rows(
  ...
  left_join(level0to3 %>% dplyr::select(gid0=gid,admin0_name_clean=name)) %>% #this may take an embarrassingly long time
  left_join(level0to3 %>% dplyr::select(gid1=gid,admin1_name_clean=name)) %>% #this may take an embarrassingly long time
  left_join(level0to3 %>% dplyr::select(gid2=gid,admin2_name_clean=name)) #%>% #this may take an embarrassingly long time
...
nytimes_long <- nytimes %>%
                ...
                left_join(level0to3 %>% dplyr::select(gid0=gid,admin0_name_clean=name)) %>% #this may take an embarrassingly long time
                left_join(level0to3 %>% dplyr::select(gid1=gid,admin1_name_clean=name)) %>% #this may take an embarrassingly long time
                left_join(level0to3 %>% dplyr::select(gid2=gid,admin2_name_clean=name))
```
...
metabiota_long <- metabiota_confirmed %>% 
                  ...
                  mutate(admin3_name_clean= ifelse(!is.na(admin3_name_clean) & admin3_name_clean !='', 
                                            paste0(admin2_name_clean,"_",admin3_name_clean),
                                            NA)
                         ) %>%
                  ...
                  left_join(level0to3 %>% dplyr::select(gid1=gid,admin1_name_clean=name)) %>% #this may take an embarrassingly long time
                  left_join(level0to3 %>% dplyr::select(gid2=gid,admin2_name_clean=name)) %>% #this may take an embarrassingly long time
                  left_join(level0to3 %>% dplyr::select(gid3=gid,admin3_name_clean=name)) #%>% #this may take an embarrassingly long time
...
WDIData <- read_csv("/media/skynet2/905884f0-7546-4273-9061-12a790830beb/rwd_github_private/NESScovid19/data_in/WDIData.csv")
