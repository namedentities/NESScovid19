...
data.table::data.table(
  Altai.region = c("Altai Republic","Amurskaya Oblast",
                   "Arkhangelsk region","Astrakhan region","Belgorod region",
                   "Bryansk region","Chechen Republic","Chelyabinsk region",
                   "Chukotka Autonomous Okrug","Chuvash Republic",
                   "Diamond Princess","Irkutsk region","Ivanovo region",
                   "Jewish Autonomous Region","Kabardino-Balkar Republic","Kaliningrad region",
                   "Kaluga region","Kamchatka Krai",
                   "Karachay-Cherkess Republic","Kemerovo region","Khabarovsk region","Khanty-Mansiysk",
                   "Kirov region","Komi Republic","Kostroma region",
                   "Krasnodar region","Krasnoyarsk region","Kurgan region",
                   "Kursk region","Leningrad region","Lipetsk region","Magadan Region",
                   "Mari El Republic","Moscow","Moscow region",
                   "Murmansk region","Nenets Autonomous Okrug","Nizhny Novgorod Region",
                   "Novgorod region","Novosibirsk region","Omsk region",
                   "Orenburg region","Oryol Region","Penza region","Perm",
                   "Primorsky Krai","Pskov region","Republic of Adygea",
                   "Republic of Bashkortostan","Republic of Crimea","Republic of Kalmykia",
                   "Republic of North Ossetia - Alania","Republic of Tatarstan",
                   "Rostov region","Ryazan Oblast","Sakhalin region",
                   "Samara Region","Saratov region","Sevastopol","Smolensk region",
                   "St. Petersburg","Stavropol region","Sverdlovsk region",
                   "Tambov Region","The Republic of Buryatia",
                   "The Republic of Dagestan","The Republic of Ingushetia",
                   "The Republic of Karelia","The Republic of Khakassia",
                   "The Republic of Mordovia","The Republic of Sakha (Yakutia)","Tomsk region",
                   "Tula region","Tver region","Tyumen region","Tyva Republic",
                   "Udmurt republic","Ulyanovsk region","Vladimir region",
                   "Volgograd region","Vologda Region","Voronezh region",
                   "Yamalo-Nenets Autonomous Okrug","Yaroslavl region","Zabaykalsky Krai"),
           ALT = c("AL","AMU","ARK","AST","BEL","BRY",
                   "CE","CHE","CHU","CU",NA,"IRK","IVA","YEV","KB",
                   "KGD","KLU","KAM","KC","KEM","KHA","KHM","KIR","KO",
                   "KOS","KDA","KYA","KGN","KRS","LEN","LIP","MAG","ME",
                   "MOW","MOS","MUR","NEN","NIZ","NGR","NVS","OMS","ORE",
                   "ORL","PNZ","PER","PRI","PSK","AD","BA",NA,"KL",
                   "SE","TA","ROS","RYA","SAK","SAM","SAR",NA,"SMO","SPE",
                   "STA","SVE","TAM","BU","DA","IN","KR","KK","MO",
                   "SA","TOM","TUL","TVE","TYU","TY","UD","ULY","VLA",
                   "VGG","VLG","VOR","YAN","YAR","ZAB")
)


openRU_long <- openRUcases %>% dplyr::select(Date, `Region/City-Eng`, Region_ID, Confirmed, Deaths) %>% 
  mutate(date_asdate = ymd(Date)) %>% 
  mutate(admin0_name_original="Russia") %>%
  mutate(admin1_name_original=`Region/City-Eng`)  %>%
  mutate(admin2_name_original='')  %>%
  mutate(admin0_name_clean=admin0_name_original %>% rex_clean()) %>%
  mutate(admin1_name_clean=admin1_name_original %>% rex_clean()) %>%
  mutate(admin2_name_clean=admin2_name_original %>% rex_clean()) %>%
