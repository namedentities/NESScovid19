...
forjoining_covid19japan <- bind_rows(covid19japan_national_long,covid19japan_prefecture_long) %>% 
                           ...
                           dplyr::select(dataset,gid, geonameid, wikidata_id, date_asdate, confirmed=confirmed, deaths=deceasedCumulative, tested_people=testedCumulative)
...
wikipedia_subnational_turkey_long <- wikipedia_subnational_turkey %>%
                                      ...
                                      mutate(confirmed = coalesce(show_all_vte_covid_19_cases_in_turkey_by_province_2) %>% str_replace_all("\\[.*?\\]|\\(.*?\\)|,","") ) %>%
                                      mutate(tested_people = coalesce(show_all_vte_covid_19_cases_in_turkey_by_province_3) %>% str_replace_all("\\[.*?\\]|\\(.*?\\)|,","")  ) %>%
...
wikipedia_subnational_tests_long <- wikipedia_subnational_tests %>%
                                  ...
                                  mutate(confirmed = coalesce(positive) ) %>%
                                  mutate(tested_people = coalesce(tests) ) %>%
