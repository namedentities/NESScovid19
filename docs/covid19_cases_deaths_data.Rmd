...
covid_19_uk_data_long <- bind_rows(
  ...
  covid_19_uk_data7
                    ...
                    mutate(confirmed = coalesce(TotalCases %>% as.numeric() ,ConfirmedCases %>% as.numeric() ) ) %>%
...
forjoining_covid_19_uk <- covid_19_uk_data_long %>%
                          dplyr::select(dataset,admin0_name_original, admin1_name_original, admin2_name_original, gid, geonameid, wikidata_id,date_asdate, confirmed=confirmed, deaths=Deaths, people_test=Tests)
...
rawdata_df$date <- ymd("2020-01-08"):(ymd("2020-01-08")+nrow(rawdata_df)-1) %>% as.Date()
...
wikipedia_subnational_south_africa_long <- pivot_longer(wikipedia_subnational_south_africa, names_to = "date", cols = starts_with("0"), values_to = "confirmed") %>% 
                                           ...
                                           mutate_if(is.character, list(~na_if(., ""))) %>% #coalesce may not handle '' correctly
...
wikipedia_subnational_turkey_long <- wikipedia_subnational_turkey %>%
                                      ...
                                      mutate(confirmed = coalesce(show_all_vte_covid_19_cases_in_turkey_by_province_2) %>% str_replace_all("\\[.*?\\]|\\(.*?\\)|,","") %>% as.numeric() ) %>%
                                      mutate(tested_people = coalesce(show_all_vte_covid_19_cases_in_turkey_by_province_3) %>% str_replace_all("\\[.*?\\]|\\(.*?\\)|,","") %>% as.numeric() ) %>%
    
                                      mutate(place0 = "Turkey" ) %>%
                                      mutate(place1 = coalesce(show_all_vte_covid_19_cases_in_turkey_by_province) ) %>%
      
                                      mutate(date = coalesce(edit_date) ) %>%
...
wikipedia_subnational_thailand_long <- wikipedia_subnational_thailand %>%
                                      ...
                                      mutate(confirmed = coalesce(cases,total) %>% str_replace_all("\\[.*?\\]|\\(.*?\\)|,","") %>% as.numeric() ) %>%

                                      mutate(place0 = "Thailand" ) %>%
                                      mutate(place1 = coalesce(province) ) %>%
...
temp <- readLines("https://raw.githubusercontent.com/microsoft/Bing-COVID-19-Data/master/data/2020-4-21.csv")
...
wikipedia_national_test_long <- wikipedia_national_test %>% 
                                ...
                                rowwise() %>% 
                                mutate(dataset="wikipedia") %>%
                                mutate(tested_people = coalesce(total_tests,totaltests_a, totaltests, vte_volume_of_testing_2, vte_volume_of_testing, vte_covid_19_testing_volume_2) ) %>%
  
                                mutate(date = coalesce(as_of,as_of_2, vte_covid_19_testing_volume_3, ) ) %>%
                                mutate(place = coalesce(country_or_region,country, vte_covid_19_testing_volume, country_or_territory, vte_volume_of_testing) ) %>% 
...
wikipedia_subnational_tests_long <- wikipedia_subnational_tests %>%
                                  ...
                                  mutate(confirmed = coalesce(positive) %>% as.character() %>% as.numeric() ) %>%
                                  mutate(tested_people = coalesce(tests) %>% as.character() %>% as.numeric()  ) %>%

                                  mutate(place0 = coalesce(country_a) ) %>%
                                  mutate(place1 = coalesce(subdivision) ) %>%
...
wikipedia_national_cases_long <- wikipedia_national_cases %>%
                                  ...
                                  mutate(confirmed = coalesce(confirmedcases,confirmed,cases,cases_b,vte2019_20_coronavirus_outbreak_by_country_and_territory_2) ) %>%
                                  mutate(deaths = coalesce(deaths_c,deaths,deaths_b,vte2019_20_coronavirus_outbreak_by_country_and_territory_3) ) %>%

                                  mutate(confirmed = confirmed %>% str_replace("[^0-9]","") %>% as.numeric() ) %>%
                                  mutate(deaths = deaths %>% str_replace("[^0-9]","") %>% as.numeric()) %>%
  
                                  mutate(place = coalesce(country_region,country_or_territory_a,locations_a,locations_a_2,country_or_region,countries_and_territories_a,country_or_territory,
...
dim(lhs_long) #398999 #310169 #304,376
