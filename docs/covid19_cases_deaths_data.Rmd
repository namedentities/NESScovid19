...
forjoining_covid19india <- covid19india_test_long %>% 
                           dplyr::select(dataset,gid, geonameid,wikidata_id,date_asdate , tested_people=totaltested)
...
library(tidyselect)
covid19japan_prefecture_long <- pivot_longer(rawdata_df, names_to = "admin1_name_original", cols = Tokyo:HIroshima, values_to = "confirmed") %>% #-ends_with("date")
...
iedcr_long <- iedcr %>% filter(!is.na(cases)) %>%
...
dim(lhs_long) #310169 #304,376

lhs_long$dataset %>% table()
